...
-- Fix: qb-policejob/client/main.lua, client/job.lua and
-- qb-ambulancejob/client/job.lua all call
-- TriggerServerEvent('police:server:UpdateBlips') right after a duty
-- toggle / position-relevant change, expecting an immediate blip refresh.
-- No RegisterNetEvent for that name existed anywhere, so those calls were
-- silently dropped. UpdateBlips() was still running on its own 5s
-- CreateThread loop below, so duty/position changes were not lost
-- permanently, only delayed up to ~5s, and the on-duty-change 'refresh now'
-- intent from those call sites did nothing. This registers the missing
-- handler and reuses the existing UpdateBlips() function as-is (same data,
-- same recipients, no new broadcast logic).
