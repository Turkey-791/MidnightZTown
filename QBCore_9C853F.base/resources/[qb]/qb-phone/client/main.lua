    ...
    QBCore.Functions.TriggerCallback('qb-phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then
            for k, _ in pairs(result) do
                -- Fix: this callback name was missing ':server:' and did not
                -- match the registered callback in qb-policejob/server/vehicle.lua
                -- (police:server:IsPlateFlagged), so it silently never returned
                -- and isFlagged was never set (defaulted to nil/false for every
                -- result, even actually-flagged plates). Did not freeze this
                -- particular callback because cb(result) below does not wait
                -- on it, but it did make flagged vehicles show as not flagged.
...
RegisterNUICallback('FetchVehicleScan', function(_, cb)
    ...
    QBCore.Functions.TriggerCallback('qb-phone:server:ScanPlate', function(result)
        -- Fix: qb-phone:server:ScanPlate (server/main.lua) calls cb(nil) when
        -- no vehicle is nearby (the 'No Vehicle Nearby' error path). The old
        -- code below unconditionally did result.isFlagged = ..., which is an
        -- index-nil error on that path, so the NUI callback 'cb' was never
        -- called and the phone's scan UI would hang forever on that specific
        -- case. Guard it and resolve the NUI callback immediately instead.
        if result == nil then
            cb(nil)
            return
        end
        -- Fix: 'police:IsPlateFlagged' did not match the registered callback
        -- name 'police:server:IsPlateFlagged' (qb-policejob/server/vehicle.lua),
        -- so this callback was never answered and cb(result) below - which is
        -- nested inside it - never ran, freezing the vehicle scan UI on every
        -- successful scan.
        QBCore.Functions.TriggerCallback('police:server:IsPlateFlagged', function(flagged)
